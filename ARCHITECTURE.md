...
Mory is not just a React app. It should become a local-first memory operating layer with clear boundaries between capture, storage, AI processing, sync, and user experience.
...
Apps
  ...
  browser-extension   pages, highlights, tabs, reading context
...
Connector Event
  ...
  -> Memory Engine Derivations
  -> Search / Recall / Prompt Context
...
- Append-only: AI never edits history; it can only add derived objects.
- Content-addressed: duplicate content is detected by stable hashes.
- Provider-neutral: OpenAI, local models and future providers are adapters.
...
- Snapshots: daily or project-level summaries generated as derived objects.
...
- Add `packages/memory-engine` for summary, tags, embeddings and recall context.
- Add OPFS-backed attachment storage.
- Add provider adapters for OpenAI and local models.
